...
clip_model = CLIPModel.from_pretrained("/root/.cache/huggingface/hub/models--openai--clip-vit-base-patch32")
clip_processor = CLIPProcessor.from_pretrained("/root/.cache/huggingface/hub/models--openai--clip-vit-base-patch32")

def _normalize_clip_image(image: torch.Tensor) -> torch.Tensor:
    ...
    image = rp.torch_resize_image(image, (224, 224))
    image = (image - mean[:, None, None]) / std[:, None, None]
    return image


def get_clip_image_features(image: torch.Tensor) -> torch.Tensor:
    """
    Returns CLIP image features for a CHW tensor with gradients preserved.
    """
    assert isinstance(image, torch.Tensor), "image should be a torch.Tensor"
    assert image.ndim == 3, "image should be in CHW format"
    assert image.shape[0] == 3, "image should be rgb"
    assert image.min() >= 0 and image.max() <= 1, "image values must be in [0, 1]"

    image = _normalize_clip_image(image)
    image = image[None]
    image = image.type_as(next(clip_model.parameters()))
    outputs = clip_model.to(image.device).get_image_features(pixel_values=image)
    features = outputs / outputs.norm(dim=-1, keepdim=True)
    return features[0]


def get_clip_image_similarity(image: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """
    Returns cosine similarity between two images in CHW format.
    """
    image_features = get_clip_image_features(image)
    target_features = get_clip_image_features(target)
    return torch.sum(image_features * target_features, dim=-1)
