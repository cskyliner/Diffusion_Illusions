...
import rp
from source.clip_new import get_clip_image_features
...
class StableDiffusion(nn.Module):
    ...
    def train_step(self, 
                   ...
                   latent_coef=0,
                   image_coef=0,
                   clip_image_target:Optional[torch.Tensor]=None,
                   clip_image_coef:float=0.0,
                  ):
        ...
        if clip_image_target is not None and clip_image_coef:
            clip_pred = pred_rgb_512[0] if pred_rgb_512.ndim == 4 else pred_rgb_512
            if isinstance(clip_image_target, torch.Tensor):
                clip_target = clip_image_target
                if clip_target.ndim == 4:
                    clip_target = clip_target[0]
                if clip_target.shape[0] != 3 and clip_target.shape[-1] == 3:
                    clip_target = clip_target.permute(2, 0, 1)
                clip_target = clip_target.to(self.device)
            else:
                clip_target = rp.as_rgb_image(clip_image_target)
                clip_target = rp.as_float_image(clip_target)
                clip_target = rp.as_torch_image(clip_target).to(self.device)

            clip_pred_features = get_clip_image_features(clip_pred)
            clip_target_features = get_clip_image_features(clip_target)
            clip_loss = 1 - torch.sum(clip_pred_features * clip_target_features, dim=-1)
            (clip_loss.mean() * clip_image_coef).backward(retain_graph=True)
